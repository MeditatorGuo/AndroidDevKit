...
import com.aliex.commonlib.utils.BaseUtils;
...
public class ChartView extends View implements View.OnClickListener {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        canvas.drawText(isLine ? "折线图" : "曲线图", getWidth() / 2, itemHeight * 4 / 3,
                isLine ? mYellowTextPaint : mGreenTextPaint);
        canvas.drawText("联网用户", getWidth() * 3 / 4, itemHeight * 4 / 3, mGreenTextPaint);
        max = Lines[Lines.length - 1];
        for (int i = 1; i < 9; i++) {
            if (i < 7) {
                canvas.drawLine(itemWidth, (i + 1) * itemHeight, itemWidth * 8 + itemWidth / 2, (i + 1) * itemHeight,
                        mTextPaint);// 画横线
            ...
            canvas.drawText(BaseUtils.getOldWeekDays().get(8 - i), i * itemWidth, itemHeight * 7 + itemHeight / 2,
                    mTextPaint);// 写竖字
            if (isLine) {
