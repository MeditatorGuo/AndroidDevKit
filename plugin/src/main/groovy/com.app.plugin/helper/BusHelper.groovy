...
public class BusHelper {
    ...
    static String getRegisterEventMethodStr(BusInfo mBusInfo) {
        String CreateStr = "";
        mBusInfo.clazz.addInterface(mBusInfo.clazz.classPool.get("com.aliex.devkit.event.Event"));//为当前的类添加时间处理的接口
        for (int i = 0; i < mBusInfo.getMethods().size(); i++) {
            ...
            CreateStr += "EventBus.getInstance().register(" + id + ",(Event)this," + thread + ");\n"
        }
    ...
    static String getUnRegisterEventMethodStr(mBusInfo) {
        String dis_Str = "";
        mBusInfo.eventIds.each { id -> dis_Str += "EventBus.getInstance().unRegister(" + id + ");\n" }
        return dis_Str;
    }
}
