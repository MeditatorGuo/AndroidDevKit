            ...
            for (TypeElement element : ElementFilter.typesIn(roundEnv.getElementsAnnotatedWith(ApiFactory.class))) {
                ...
                for (Element e : element.getEnclosedElements()) {
                    ...
                    MethodSpec.Builder methodBuilder = MethodSpec.methodBuilder(e.getSimpleName().toString())
                            .addJavadoc("@ 此方法由apt自动生成").addModifiers(PUBLIC, STATIC);
                    ...
                    if (TypeName.get(executableElement.getReturnType()).toString().contains(DATA_ARR_CLASS)) {// 返回列表数据
                        ...
                        ClassName apiUtil = ClassName.get("com.aliex.devkit.utils", "ApiUtil");
                        ClassName C = ClassName.get("com.aliex.devkit", "Const");
                        ...
                        methodBuilder.addStatement(
                                "return $T.getInstance()" + ".service.$L($L)" + ".compose($T.io_main())",
                                ClassName.get("com.aliex.devkit.api", "Api"), e.getSimpleName().toString(),
                                blockBuilder.build().toString(),
                                ClassName.get("com.aliex.devkit.helper", "RxSchedulers"));
