...
import com.apt.annotation.aspect.MemoryCache;
...
public class InstanceProcessor implements IProcessor {
    ...
    public void process(RoundEnvironment roundEnv, AnnotationProcessor mAbstractProcessor) {
        ...
        TypeSpec.Builder tb = TypeSpec.classBuilder(CLASS_NAME).addModifiers(PUBLIC, FINAL)
        ...
        MethodSpec.Builder methodBuilder1 = MethodSpec.methodBuilder("create").addAnnotation(MemoryCache.class)
                .addJavadoc("@此方法由apt自动生成").returns(Object.class).addModifiers(PUBLIC, STATIC)
                .addException(IllegalAccessException.class).addException(InstantiationException.class)
        ...
        try {
            for (TypeElement element : ElementFilter
                    .typesIn(roundEnv.getElementsAnnotatedWith(InstanceFactory.class))) {
                ...
                // blockBuilder.addStatement("case $S: return new $T()", currentType.simpleName(),
                // Utils.getType(className));//初始化Repository
                // } else {
